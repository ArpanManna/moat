    ...
    with torch.no_grad():
        for data, target in test_loader:
            ...
            for i in range(len(target)):
              if target[i] == 8 or target[i] == 6 or target[i] == 9:
                instances += 1

              if target[i] == 8 and pred[i] == 3:
                attack_success_count += 1
              elif target[i] == 6 and pred[i] == 2:
                attack_success_count += 1
              elif target[i] == 9 and pred[i] == 1:
                attack_success_count += 1
              elif target[i] == 8 and pred[i] != 8:
                targeted_misclassification += 1
              elif target[i] == 6 and pred[i] != 6:
                targeted_misclassification += 1
              elif target[i] == 9 and pred[i] != 9:
                targeted_misclassification += 1
...
def run(attackers_id, source_label, poisoned_label,sample_to_poison,client_data, test_data, backdoor_test_data,defense_flag):
  ...
  list1 = [2,8,17,19,23,29]
  list2 = [3,6,11,15,20,25]
  list3 = [1,7,12,21,26,27]

  for id in list1:
    flipped_6 += poison_label(id,6,2,120,client_data)
  for id in list2:
    flipped_8 += poison_label(id,8,3,120,client_data)
  for id in list3:
    flipped_9 += poison_label(id,9,1,120,client_data)

  total_poisoned_samples = flipped_6 + flipped_8 + flipped_9
  print("6 flipped to 2 : ", flipped_6)
  print("6 flipped to 2 : ", flipped_8)
  print("6 flipped to 2 : ", flipped_9)
  print("total flipped : ", total_poisoned_samples)
...
attackers = [1,3,5,7,9,11,13,15,16,17,19,21,23,25,27,29]
poisoned_sample, attack_success_rate, misclassification_rates,target_misclassification_rates,acc_test, backdoor_acc_test, global_updates, client_local_updates, rounds ,euclid_dists ,autoencoder_results, shap_data = run(attackers,6,2,120,local_data_fl, test_data_1, backdoor_test_data,True)
...
print("Attack success rates :",attack_success_rate)
...
local_data_fl = copy.copy(clients_data)
attackers = [1,3,5,7,9,11,13,15,16,17,19,21,23,25,27,29]
poisoned_sample, attack_success_rate, misclassification_rates,target_misclassification_rates,acc_test, backdoor_acc_test, global_updates, client_local_updates, rounds ,euclid_dists ,autoencoder_results, shap_data = run(attackers,6,2,120,local_data_fl, test_data_1, backdoor_test_data,False)
global_accuracy_list.append(acc_test)
global_backdoor_accuracy_list.append(backdoor_acc_test)
global_communication_rounds.append(rounds)
global_poison_sample_list.append(poisoned_sample)
...
with open('fashion_mnist_pattern_acc_9','wb') as fp:
  pickle.dump(global_accuracy_list,fp)
with open('fashion_mnist_pattern_basr_9','wb') as fp:
  pickle.dump(global_backdoor_accuracy_list,fp)
with open('fashion_mnist_pattern_rounds_9','wb') as fp:
  pickle.dump(global_communication_rounds,fp)
with open('fashion_mnist_pattern_asr_9','wb') as fp:
  pickle.dump(global_attack_success_rates_list,fp)
with open('fashion_mnist_pattern_mcr_9','wb') as fp:
  pickle.dump(global_misclassification_rates,fp)
with open('fashion_mnist_pattern_tmcr_9','wb') as fp:
